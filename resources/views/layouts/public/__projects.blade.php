<article>
    ...
    <!-- Project Image -->
    <div class="post-img">
        <a href="{{ route('project.details', $project->id) }}"><img src="{{ $imagePath }}" alt="{{ $project->title }}" class="w-100 rounded-3" style="height: 250px; object-fit: cover;"></a>
    </div>
    ...
    <div class="post-engagement d-flex align-items-center gap-3">
        ...
        <button class="btn btn-sm like-btn d-flex align-items-center gap-1 guest-action" style="border-width: none; background: transparent;" data-action="like" data-project-id="{{ $project->id }}">
            <i class="bi {{ $project->liked_by_auth_user ? 'bi-heart-fill text-danger' : 'bi-heart text-muted' }} fs-4"></i>
